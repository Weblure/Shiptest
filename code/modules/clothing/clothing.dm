...
/obj/item/proc/generate_species_clothing(file2use, state2use, layer2use, datum/species/mob_species)
	if(!icon_exists(file2use, state2use))
		return

	if(generate_species_offsets(file2use, state2use, layer2use, mob_species))
		return TRUE

	var/icon/human_clothing_icon = icon(file2use, state2use)

	if("[layer2use]" in mob_species.offset_clothing)
		// This code taken from Baystation 12
		var/icon/final_I = icon('icons/blanks/32x32.dmi', "nothing")
		var/list/shifts = mob_species.offset_clothing["[layer2use]"]
...
/proc/generate_species_offsets(file2use, state2use, layer, datum/species/mob_species)
